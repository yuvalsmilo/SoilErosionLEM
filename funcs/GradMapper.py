...
class GradMapper(Component):
    ...
    def __init__(
        ...
        self._nodes_flatten = self._grid.nodes.flatten()
        self._sigma = sigma
        self._phi = phi
        self._topo0 = np.copy(self._grid.at_node['topographic__elevation'])
    ...
    def run_one_step(self,):
        ...
        ## REMMEBER THAT DIRECTION OF TOPOGRAPHIC GRADIENT AT LINK IS IN OPPOSITE TO FLUX DIRECTION
        ...
        self._grid.at_node['water_surface__elevation'][self._grid.core_nodes] =  self._grid.at_node['surface_water__depth'][self._grid.core_nodes] + self._grid.at_node['topographic__elevation'][self._grid.core_nodes]


        water_gradient_at_link = self._grid.calc_grad_at_link('water_surface__elevation')
        gradient_to_outlet = topographic_gradient_at_link[self._outlet_links] #! ACCORDING TO THE TOPOGRAPHIC SLOPE AND NOT THE WATER SLOPE.
    ...
    def flux_mapper(self,
        ...
        "get suspended sediment fluxes [kg/s]"
        total_suspended_incoming_flux = np.copy(self._grid.at_node['sediment__influx'])  


        return total_suspended_incoming_flux